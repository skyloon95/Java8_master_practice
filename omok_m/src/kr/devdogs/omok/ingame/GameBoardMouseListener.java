...
public class GameBoardMouseListener implements MouseListener,MouseMotionListener{
	...
	public int averageFinderX(int n) {
		int tmp = n - (int)(Main.BOARDLINE_START_WIDTH);
		int divvy = tmp%(int)(Main.LINE_SPACE);	//	몫
		int residuum = tmp/(int)(Main.LINE_SPACE);	//	나머지

		int nearPoint;

		nearPoint = (divvy < ((int)(Main.LINE_SPACE)/2)) ? residuum : residuum+1;

		return nearPoint;
	}

	public int averageFinderY(int n) {
		int tmp = n - (int)(Main.BOARDLINE_START_HEIGHT);
		int divvy = tmp%(int)(Main.LINE_SPACE);	//	몫
		int residuum = tmp/(int)(Main.LINE_SPACE);	//	나머지
	...
	public boolean isInside(int x, int y) {
		return x>=0&&x<(Main.LINE)&&y>=0&&y<(Main.LINE);
	}
	...
	private int getCoordinateX(int x) {
		return (int)(Main.BOARDLINE_START_WIDTH + (x * Main.LINE_SPACE)-(Main.STONE_SIZE/2));
	}

	private int getCoordinateY(int y) {
		return (int)(Main.BOARDLINE_START_HEIGHT + (y * Main.LINE_SPACE)-(Main.STONE_SIZE/2));
	}
